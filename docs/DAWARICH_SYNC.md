...
Der initiale Abgleich verarbeitet die vorhandenen Punkte batchweise und nutzt
eine stabile Keyset-Grenze. Danach werden Änderungen inkrementell verarbeitet.
Bei einem beschädigten oder verlorenen Cursor kann ein vollständiger Abgleich
durch Zurücksetzen des Sync-Status und Neustart des Dienstes durchgeführt
werden. Stand der letzten lokalen Verifikation am 06.09.2026: `486.210`
Dawarich-Punkte im Receiver-Mirror, Cursor auf Event-ID `72.325`,
`last_error` leer. Die Spiegelung verwendet eine eigene Zuordnungstabelle; eine
relationale Foreign-Key-Verknüpfung zu `gps_points` ist derzeit nicht
eingerichtet.
